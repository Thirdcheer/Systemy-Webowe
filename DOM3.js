function altIsPressed(event) {
    if (event.altKey) {
        window.alert("You pressed ALT while clicking on gallery!");
    } else {
        window.alert("You did not press ALT while clicking on gallery!");
    }
}

function ctrIsPressed(event){
    if (event.ctrlKey) {
        SnowStart();
    }
}

function shiftIsPressed(event){
    if (event.shiftKey) {
        window.alert("⊂_ヽ \n" +
            "　 ＼＼ Λ＿Λ \n" +
            "　　 ＼('ㅅ') \n" +
...
function getUnicode(event) {
    ...
    document.getElementById("show_unicode").innerHTML = "Last KeyCode: " + key;
}
...
function coords(event){
    ...
    document.getElementById("show_unicode").innerHTML = coord;
}
